...
const VS = [
  ...
  "  gl_Position = (z < 0.0 || f > 3.5) ? vec4(2.0, 2.0, 2.0, 1.0) : vec4(c.x, -c.y, 0.0, 1.0);",   // 4 = schovaná filtrem
  "}"].join("\n");
const FS = [
...
function klikDoMapy(e){
  ...
  const zde = (V_ZEMI[nalezena.properties.name] || []).filter(function(j){ return !atlasSkryty(j); });
  okno.textContent = "";
